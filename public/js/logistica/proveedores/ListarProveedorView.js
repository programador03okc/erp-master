        ...
        construirTablaCuentaBancariaProveedor(data){
            if(data.length >0){
                (data).forEach(element => {
                    ...
                    <td><input type="hidden" name="idBanco[]" value="${(element.idBanco !=null && element.idBanco !='')?element.idBanco:''}"><input type="hidden" name="nombreBanco[]" value="${(element.nombreBanco !=null && element.nombreBanco !='')?element.nombreBanco:''}"> ${(element.nombreBanco !=null && element.nombreBanco !='')?element.nombreBanco:''}</td>
            ...
            if(mensaje.length >0){
                ...
                $.ajax({
                    ...
                    beforeSend:  (data)=> { // Are not working with dataType:'jsonp'
                        ...
                        $('#mmodal-proveedor .modal-content').LoadingOverlay("show", {
                            imageAutoResize: true,
                            progress: true,
                            imageColor: "#3c8dbc"
                        });
                    ...
                    success: (response) =>{
                        ...
                        if (response.id_proveedor > 0) {
                            $('#mmodal-proveedor .modal-content').LoadingOverlay("hide", true);
                            ...
                            Swal.fire(
                                '',
                                'Lo sentimos hubo un problema en el servidor al intentar guardar el proveedor, por favor vuelva a intentarlo',
                                'error'
                            );
                            obj.removeAttribute("disabled");
