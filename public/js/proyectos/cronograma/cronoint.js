...
function nuevo_cronoint(){
    console.log('nuevo_cronoint');
...
function mostrar_lista(){
    var html = '';
    var disabled = '';
    var type = $("#form-cronoint").attr('type');
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                if (response > 0){
					...
					$('#form-cronoint').attr('type', 'register');
					changeStateInput('form-cronoint', true);
                }
            }
        }).fail( function( jqXHR, textStatus, errorThrown ){
            console.log(jqXHR);
...
function anular_cronoint(id_presupuesto){
    $.ajax({
        type: 'GET',
