...
function actualizaCantidadDespachosTabs() {
    $.ajax({
        ...
        success: function(response) {
            ...
            $("#selaborados").text(
                response["count_pendientes"] > 0
                    ? response["count_pendientes"]
                    : ""
            );
            $("#sconfirmados").text(
                response["count_confirmados"] > 0
                    ? response["count_confirmados"]
                    : ""
            );
            $("#spendientes").text(
                response["count_en_proceso"] > 0
                    ? response["count_en_proceso"]
                    : ""
            );
            $("#stransformados").text(
                response["count_en_transformacion"] > 0
                    ? response["count_en_transformacion"]
                    : ""
            );
            $("#sdespachos").text(
                response["count_por_despachar"] > 0
                    ? response["count_por_despachar"]
                    : ""
            );
            $("#ssinTransporte").text(
                response["count_despachados"] > 0
                    ? response["count_despachados"]
                    : ""
            );
            $("#sretornoCargo").text(
                ...
                render: function(data, type, row) {
                    ...
                    return (
                        ...
                        (row["estado"] == 19 &&
                            ...
                            ? `<button type="button" class="adjuntar btn btn-${
                                  row["count_despacho_adjuntos"] > 0
                                      ? "warning"
                                      : "default"
...
function listarOrdenesPendientes() {
    ...
    tableDespachos = $("#ordenesDespacho").DataTable({
        ...
        columns: [
            ...
            {
                render: function(data, type, row) {
                    return `<button type="button" class="adjuntar btn btn-${
                        row["count_despacho_adjuntos"] > 0
                            ? "warning"
                            : "default"
                    } boton" data-toggle="tooltip" 
