...
function actualizaCantidadDespachosTabs(){
    $.ajax({
        ...
        success: function(response){
            ...
            $('#stransformados').text(response['count_en_transformacion'] > 0 ? response['count_en_transformacion'] : '');
...
$('#requerimientosEnTransformacion tbody').on("click","button.despacho", function(){
    var data = $('#requerimientosEnTransformacion').DataTable().row($(this).parents("tr")).data();
    console.log(data);
    tab_origen = 'enTransformacion';
    open_despacho_create(data);
});

$('#requerimientosEnTransformacion tbody').on("click","button.anular_od", function(){
    var id = $(this).data('id');
    var cod = $(this).data('cod');
    var msj = confirm('¿Está seguro que desea anular la '+cod+' ?');
    if (msj){
