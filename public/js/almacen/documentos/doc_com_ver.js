    ...
    $.ajax({
        ...
        success: function(response){
            ...
            response['docs'].forEach(element => {
                ...
                var i = 1;
                let detalles = response['detalles'].filter(detalle => detalle.id_doc == element.id_doc_com);
                ...
                detalles.forEach(item => {
                    html += `<tr>
                        ...
                        <td class="right">${formatNumber.decimal(item.precio_total,'',-2)}</td>
                    </tr>`;
                    i++;
                });
                html += `<tr>
                    <td colSpan="11" class="right">SubTotal</td>
                    <th class="right">${formatNumber.decimal(element.sub_total,element.simbolo,-2)}</th>
                </tr>
                <tr>
                    <td colSpan="11" class="right">IGV</td>
                    <th class="right">${formatNumber.decimal(element.total_igv,element.simbolo,-2)}</th>
                </tr>
                <tr>
                    <td colSpan="11" class="right">Total</td>
                    <th class="right">${formatNumber.decimal(element.total_a_pagar,element.simbolo,-2)}</th>
                </tr>
                <tr><td colSpan="12"></td></tr>`;
            });
