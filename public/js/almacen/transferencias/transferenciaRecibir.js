    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            response.forEach(element => {
                ...
                <td><input type="number" class="input-data right" style="width:80px;" name="cantidad_recibida" value="${
                    element.cantidad
                }" max="${element.cantidad}" data-idtra="${
                    element.id_trans_detalle
                }"/></td>
...
function recibir() {
    ...
    if (estado !== "14" && estado !== "7") {
        ...
        $(
            "#listaTransferenciaDetalleRecibir input[type=checkbox]:checked"
        ).each(function() {
            // console.log($(this).parent().parent()[0].data('idtra'));
            var nuevo = {
                id_guia_ven_det: $(this)
                    .parent()
                    .parent()[0].id,
                id_trans_detalle: $(this)
                    .closest("td")
                    .siblings()
                    .find("input[name=cantidad_recibida]")
                    .data("idtra"),
                cantidad_recibida: $(this)
                    .closest("td")
                    .siblings()
                    .find("input[name=cantidad_recibida]")
                    .val(),
